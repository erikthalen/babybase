...
export function cameraScript(
  ...
  const ix = initialCamera?.x ?? 0;
  const iy = initialCamera?.y ?? 0;
  const iz = initialCamera?.z ?? 1;
  return html`<script>
    (function () {
      ...
      // ── Camera ────────────────────────────────────────────────
      var camera = { x: ${ix}, y: ${iy}, z: ${iz} };

      var _saveTimer = null;
      function saveCamera() {
        clearTimeout(_saveTimer);
        _saveTimer = setTimeout(function () {
          fetch("${basePath}/schema/camera", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ x: camera.x, y: camera.y, z: camera.z }),
          }).catch(function () {});
        }, 500);
      }

      function saveTablePosition(tableName, x, y) {
        fetch("${basePath}/schema/table-positions", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ tableName: tableName, x: x, y: y }),
        }).catch(function () {});
      }

      function applyCamera() {
        var tx = camera.z * camera.x;
        var ty = camera.z * camera.y;
        wrap.style.transform =
          "matrix(" + camera.z + ",0,0," + camera.z + "," + tx + "," + ty + ")";
      ...
      window.addEventListener("pointerup", function () {
        if (isDragging) {
          ...
          var tbl = dragEl.dataset.table;
          var p = pos[tbl];
          if (p) saveTablePosition(tbl, p.x, p.y);
          isDragging = false;
          dragEl = null;
        }
        if (isPanning) {
