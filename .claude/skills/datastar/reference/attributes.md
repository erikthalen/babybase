...
<!-- Fire only once -->
<div data-on-intersect.once="@get('/analytics/view')"></div>
```
...
```html
<div data-on-signal-patch="console.log('Signal changed')"></div>
```
...
```html
<div
  data-on-signal-patch-filter="count"
...
<div data-signals="{count: 0, name: '', items: []}">
  ...
  <div data-signals="{_menuOpen: false, searchQuery: ''}"></div>
</div>
```
...
```html
<div data-json-signals='{"user": {"name": "Alice", "id": 123}}'></div>
```
...
<div data-style:color="textColor">
  <div data-style:display="isVisible ? 'block' : 'none'"></div>
</div>
```
...
<button data-on:click="@post('/api', {headers: {'X-Custom': 'value'}})">
  ...
  <button data-on:click="@post('/api', {include: 'user.*'})">
    <button data-on:click="@post('/api', {exclude: '_*'})"></button>
  </button>
</button>
```
